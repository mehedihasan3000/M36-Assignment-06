    ...
    return (
        <>
            ...
            <div className='flex justify-center items-center gap-3 mt-5'>
                <button onClick={() => setToggle('product')} className={`btn px-7 rounded-full ${toggle === 'product' && 'bg-linear-to-r from-[#4F39F6] to-[#9514FA] text-white'}`}>Products</button>

                <button onClick={() => setToggle('cart')} className={`btn px-7 rounded-full ${toggle === 'cart' && 'bg-linear-to-r from-[#4F39F6] to-[#9514FA] text-white'}`}>Cart({carts.length})</button>
            </div>
